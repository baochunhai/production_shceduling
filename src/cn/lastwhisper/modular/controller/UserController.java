...
public class UserController {
	...
	public GlobalResult showName() {
		...
		if (UserUtils.getSessionUser() == null) {
			result = GlobalResult.build(400, "用户未登录");
		} else {
			result = GlobalResult.build(200, UserUtils.getSessionUser().getUser_name());
		}
		return result;
	}
	...
	public String logout() {
		...
		Log log = new Log();
		log.setOperatedate(new Date());
		log.setOperateor(UserUtils.getSessionUser().getUser_code());
		log.setOperateresult("正常");
	...
	public void userexport(User user, HttpServletResponse response) {
		String filename = "Users_exportBy" + UserUtils.getSessionUser().getUser_name() + ".xls";
		// 响应对象
		try {
			// 设置输出流,实现下载文件
